<?php
namespace Liuwen\Alipay;

defined('AOP_SDK_WORK_DIR') or define('AOP_SDK_WORK_DIR','/tmp');
...
class AopClient {
	...
	protected function curl($url, $postFields = null) {
		...
		if (curl_errno($ch)) {
			...
			throw new AopException(curl_error($ch), 0);
		} else {
			$httpStatusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
			if (200 !== $httpStatusCode) {
				throw new AopException($reponse, $httpStatusCode);
			}
		}
	...
	public function log($perfixPath,$fileFormat = 'aop_err.log'){
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$log_file = rtrim(AOP_SDK_WORK_DIR, '\\/') . '/' . "logs/aop_comm_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
		$logData = array(
			date("Y-m-d H:i:s"),
			$apiName,
			$this->appId,
        ...
        error_log($log_file,3,join('|',$logData)."\r\n");
        return true;
	}
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (strcasecmp($this->fileCharset, $this->postCharset)) {
			...
			throw new AopException("文件编码：[" . $this->fileCharset . "] 与表单提交编码：[" . $this->postCharset . "]两者不一致!");
		}
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			if ($this->checkEmpty($apiParams['biz_content'])) {
				...
				throw new AopException(" api request Fail! The reason : encrypt request is not supperted!");
			}

			if ($this->checkEmpty($this->encryptKey) || $this->checkEmpty($this->encryptType)) {

				throw new AopException(" encryptType and encryptKey must not null! ");
			}

			if ("AES" != $this->encryptType) {

				throw new AopException("加密类型只支持AES");
			}

			// 执行加密
			$enCryptContent = AopEncypt::encrypt($apiParams['biz_content'], $this->encryptKey);
...
//	}
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty($signData->signSourceData)) {
				...
				throw new AopException(" check sign Fail! The reason : signData is Empty");
			}
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				if (!$checkResult) {
					...
					if (strpos($signData->signSourceData, "\\/") > 0) {
						...
						if (!$checkResult) {
							throw new AopException("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
						}
	...
	private function encryptJSONSignSource($request, $responseContent) {
		...
		$bizContent = AopEncypt::decrypt($parsetItem->encryptContent, $this->encryptKey);
